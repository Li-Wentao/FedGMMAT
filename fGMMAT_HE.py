...
# create TenSEALContext for fedagg Sigma_iX
HE1 = ts.context(ts.SCHEME_TYPE.CKKS, poly_mod_degree, coeff_mod_bit_sizes)
HE1.global_scale = 2 ** 21
HE1.generate_galois_keys()
# create TenSEALContext for fedagg XSigma_iX
HE2 = ts.context(ts.SCHEME_TYPE.CKKS, poly_mod_degree, coeff_mod_bit_sizes)
HE2.global_scale = 2 ** 21
HE2.generate_galois_keys()
# create TenSEALContext for fedagg SCORE
HE3 = ts.context(ts.SCHEME_TYPE.CKKS, poly_mod_degree, coeff_mod_bit_sizes)
HE3.global_scale = 2 ** 21
HE3.generate_galois_keys()
# create TenSEALContext for fedagg pG
HE4 = ts.context(ts.SCHEME_TYPE.CKKS, poly_mod_degree, coeff_mod_bit_sizes)
HE4.global_scale = 2 ** 21
HE4.generate_galois_keys()
# create TenSEALContext for fedagg VAR
HE5 = ts.context(ts.SCHEME_TYPE.CKKS, poly_mod_degree, coeff_mod_bit_sizes)
HE5.global_scale = 2 ** 21
HE5.generate_galois_keys()
...
class FedGLMMkin:
    ...
    def fit(self, max_itr=100, tol = 1e-5):
        ...
        for step in range(self.max_itr):
            ...
            # calculate Sigma_iX #
            ######################
            start = 0; end = 0; Sigma_iX_enc = 0
            for i in range(len(self.X)):
                end += nx[i]
                # encryption
                plain = torch.mm(Sigma_i[start:end].T, self.X[i].double())
                encryption = ts.ckks_vector(HE1, plain.detach().flatten()*(10**6))
                # HE addition
                Sigma_iX_enc += encryption
                start = end
            Sigma_iX = torch.tensor(Sigma_iX_enc.decrypt(), dtype=torch.float64).reshape(plain.shape)/(10**6)
#             
            #### Fed Aggregate ####
            # calculate XSigma_iX #
            #######################
            start = 0; end = 0; XSigma_iX_enc = 0
            for i in range(len(self.X)):
                end += nx[i]
                # encryption
                plain = torch.mm(self.X[i].double().T, Sigma_iX[start:end])
                encryption = ts.ckks_vector(HE2, plain.detach().flatten()*(10**6))
                # HE addition
                XSigma_iX_enc += encryption
                start = end
            XSigma_iX = torch.tensor(XSigma_iX_enc.decrypt(), dtype=torch.float64).reshape(plain.shape)/(10**6)
            # calculate cov
            cov = torch.cholesky_inverse(torch.linalg.cholesky(XSigma_iX))
            # calculate sigma_iXcov
            Sigma_iXcov = torch.mm(Sigma_iX, cov)
...
def FedGLMMscore(model, geno_list):
    if (model.model_class == "fedglmmkin"):
        ######### Fed Aggregate ####
        #  calculate SCORE and pG  #
        ############################
        start = 0; end = 0; SCORE_enc = 0; pG_enc = 0; nx = [len(i) for i in model.X]
        for i in range(len(nx)):
            ...
            ## SCORE ##
            # encryption             
            SCORE_plain = torch.mm(g, model.scale_residules[start:end])
            SCORE_encryption = ts.ckks_vector(HE3, SCORE_plain.detach().flatten()*(10**6))
            # HE addition
            SCORE_enc += SCORE_encryption
            
            ## pG ##
            # encryption
            pG_plain = torch.mm(model.P[start:end].T, g.T)
            pG_encryption = ts.ckks_vector(HE4, pG_plain.detach().flatten()*(10**6))
            # HE addition
            pG_enc += pG_encryption
            start = end
        # Central Server decryption
        SCORE = torch.tensor(SCORE_enc.decrypt(), dtype=torch.float64).reshape(SCORE_plain.shape)/(10**6)
        pG = torch.tensor(pG_enc.decrypt(), dtype=torch.float64).reshape(pG_plain.shape)/(10**6)
        
        #### Fed Aggregate ####
        #    calculate VAR    #
        #######################
        start = 0; end = 0; VAR_enc = 0
        for i in range(len(nx)):
            end += nx[i]
            g = torch.tensor(geno_list[i].fillna(0).values, dtype=torch.float64)
            if len(g.shape) == 1:
                g = g.reshape([1,len(g)])
            # encryption   
            VAR_plain = torch.mm(g, pG[start:end])
            VAR_encryption = ts.ckks_vector(HE5, VAR_plain.detach().flatten()*(10**6))
            # HE addition
            VAR_enc += VAR_encryption
            start = end
        # Central Server decryption
        VAR = torch.tensor(VAR_enc.decrypt(), dtype=torch.float64).reshape(VAR_plain.shape)/(10**6)
